...
# # CONFIG
# # ----------------------------
# MODEL_PATH = "/teamspace/studios/this_studio/Designer/LLM/Qwen3-8B-Q4_K_M.gguf"
...
# Default: do NOT stream chunks.
# If you want live chunks, set STREAM_OUTPUT=True (they'll print to STDERR).
STREAM_OUTPUT = True
...
# ----------------------------
# OUTPUT SANITIZATION + JSON EXTRACTION
# ----------------------------
def sanitize_model_output(text: str) -> str:
    """Remove reasoning tags / template artifacts that sometimes leak into output."""
...
def extract_first_json_object(text: str) -> dict:
    """Extract the first valid JSON object from a noisy string.

    This is robust against:
    - leading/trailing junk like </think>
    - extra tokens after the JSON
...
for output in llm(
    ...
    chunk = output['choices'][0]['text']
    if STREAM_OUTPUT:
        # Stream chunks to STDERR so stdout redirects (>) don't capture model text.
...
try:
    layout_json = extract_first_json_object(full_response)
    print("✓ Successfully parsed JSON")
    ...
    # Ensure output filename is .json
    out_path = Path(OUTPUT_JSON_PATH)
    if out_path.suffix.lower() != ".json":
        out_path = out_path.with_suffix(".json")

    out_path.write_text(json.dumps(layout_json, indent=2, ensure_ascii=False), encoding="utf-8")
    ...
    print(f"✗ Error parsing JSON: {e}")
    print("\nRAW OUTPUT (sanitized preview):")
    print(sanitize_model_output(full_response)[:2000])
    
    # Save raw output for debugging
    debug_file = Path("raw_output_debug.txt")
    debug_file.write_text(full_response, encoding="utf-8")
    print(f"\n✓ Raw output saved to {debug_file} for debugging")
